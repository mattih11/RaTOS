...
The SDK is built from `ratos-dev-image` and includes the full RaTOS library stack:

| Package | Provides |
|---|---|
| `linux-headers-xenomai-4` | EVL kernel headers (`<evl/thread.h>` etc.) |
| `libevl` | EVL user-space library headers + `libevl.so` / `libevl.a` |
| `libcorerat-dev` | CoreRaT headers + CMake config (`CoreRaT::corerat`) |
...
| `libcommrat-dev` | CommRaT headers (available for SDK builds, not present in the runtime image) |
